import { Col, Row, Card, Button, Descriptions, Tooltip, List, Affix, Modal, Input, Radio, Icon } from 'antd';
...
class Notice extends Component {
  ...
  render() {
    ...
    // 发布公告对话框中的提示内容
    let tipsWord = <span className={styles.tip} style={{ color: 'red' }}><Icon type="exclamation-circle" style={{ margin: '0 5px' }}/>请先选择发送的公告类型</span>;
    switch (postType) {
      case 1:
        tipsWord = <span className={styles.tip}>把公告发送给Web后台，只有Web后台的设计师和管理员能看到这则公告</span>;
        break;
      case 2:
        tipsWord = <span className={styles.tip}>把公告发送给App应用，App应用的用户将看到这则公告</span>;
        break;
      default:
        break;
    }
    ...
    // 显示的主体内容
    let contentReal = <div />;
    if (data.length !== 0) { // 当获取到的data为长度不为0时（历史上存在公告）
      contentReal = (
        <React.Fragment>
          <Row gutter={24}>
    ...
    } else { // 当获取到的data为长度为0时（历史上没有任何公告）
      contentReal = (
      <React.Fragment>
        <Row gutter={24}>
          <Col>
